...
export class BrowserArtifact {
    constructor(
        ...
        /**
         * Extension of the artifact. If unspecified, derived from the download
         * URL.
         */
...
export function detectArtifactExtension(filePath: string): string {
    ...
    while ((currentPart = path.extname(tmpPath)) !== '') {
        // An extension needs to be a continuous set of alphanumeric characters.
        // This is a rather strict requirement as technically extensions could
        // contain e.g. `dashes`. In our case this strictness is acceptable
        // though as we don't expect such extensions and it makes this extension
        // detection logic more correct. e.g. the logic would not incorrectly
        // detect an extension for `firefox-97.0-linux.tar.gz` to
        // `0-linux.tar.gz`.
        if (!/^\.[a-zA-Z0-9]+$/.test(currentPart)) break;
    ...
    throw new Error(`Unable to find known extension for file path: ${
        filePath}`);
}
