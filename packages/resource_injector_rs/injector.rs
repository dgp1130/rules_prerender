...
use std::pin::Pin;
use std::rc::Rc;
use html5ever::{Attribute, Namespace, QualName, parse_document, serialize};
...
static W3_NAMESPACE: &str = "http://www.w3.org/1999/xhtml";
...
pub fn inject(html: String, config: InjectorConfig) -> Pin<Box<dyn Future<Output = Result<String, Box<dyn Error>>>>> {
    Box::pin(async move {
        ...
        eprintln!("Document: {:?}", &dom.document); // DEBUG
        ...
        for action in config {
            match action {
                InjectorAction::Script { path } => {
                    ...
                    // eprintln!("Head2: {:?}", &head); // DEBUG
                },
                InjectorAction::Style { path } => {
                    inject_style(&head, &path);
                },
            };
        }
...
fn inject_style(head: &Rc<Node>, path: &str) {
    // TODO
}
