...
_ASPECT_LABEL_ATTRS = [
    # From `prerender_component()`.
    "prerender",
    "scripts",
    "styles",
    "resources",

    # From `alias_with_metadata()`.
    "actual",
...
# Label *list* attributes which may point at `PrerenderMetadataInfo` targets.
_ASPECT_LABEL_LIST_ATTRS = ["deps"]

def _collect_transitive_metadata_aspect_impl(target, ctx):
    # Read `PrerenderMetadataInfo` off the current target if present.
...
_collect_transitive_metadata_aspect = aspect(
    ...
    attr_aspects = _ASPECT_LABEL_ATTRS + _ASPECT_LABEL_LIST_ATTRS,
    doc = """
        Collects all transitive `PrerenderMetadataInfo` providers which can be
        reached and returns them in a `_CollectedPrerenderMetadataInfo`.
    """,
)
...
_collect_transitive_styles = rule(
    ...
    doc = """
        Collects all the styles used as transitive dependencies of this target.
        Provides a `CssInfo` with all the CSS files and a `CssImportMapInfo`
        with the full import map.
    """,
)
...
_collect_transitive_scripts = rule(
    ...
    doc = """
        Collects all the scripts used as transitive dependencies of this target.
        Provides a `JSInfo` with all the JS files.
    """,
)
...
_collect_transitive_resources = rule(
    ...
    doc = """
        Collects all the resources used as transitive dependencies of this
        target. Provides a `WebResourceInfo` with all the resources.
    """,
)
