...
For `@web/test-runner`, need to patch
`node_modules/@web/config-loader/src/importOrRequireConfig.js` to look like:
...
`await getPackageType()` runs into an infinite loop looking for a `package.json`
file and failing to find it due to Bazel directory structures. This avoids it
for `.mjs` files, which is all ESM files generated by `ts_library()` anyways.
