...
import styles from 'rules_prerender/examples/site/components/base/base.css';
 ...
 *     document. Will be wrapped in a `<main />` tag, so there is no need to add
 *     one in the callback.
 */
export function baseLayout(title: string, main: () => string): string {
    return `
        <!DOCTYPE html>
        <html class="${styles.html}">
            <head>
                <title>${title}</title>
                <meta charset="utf-8">
            </head>
